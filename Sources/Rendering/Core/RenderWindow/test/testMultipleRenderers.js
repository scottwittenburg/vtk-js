...
test.onlyIfWebGL('Test multiple renderers', (t) => {
  ...
  upperRenderer.setViewport(0, 0, 1, 0.5); // xmin, ymin, xmax, ymax
  ...
  lowerLeftRenderer.setViewport(0, 0.5, 0.5, 1); // xmin, ymin, xmax, ymax
  renderWindow.addRenderer(lowerLeftRenderer);
  lowerLeftRenderer.setBackground(0, 0, 0);
  ...
  lowerRightRenderer.setViewport(0.5, 0.5, 1, 1); // xmin, ymin, xmax, ymax
  renderWindow.addRenderer(lowerRightRenderer);
  lowerRightRenderer.setBackground(1, 1, 1);
  ...
  testUtils.compareImages(image, [baseline], 'Rendering/Core/RenderWindow/testMultipleRenderers', t, 2.5, gc.releaseResources);
});
